...
function StaffHeader() {
  ...
  return (
    <>
      <AppBar position="fixed" sx={{ backgroundColor: '#e8801eff', zIndex: (theme) => theme.zIndex.drawer + 1 }}>
        <Container maxWidth="xl">
          <Toolbar disableGutters>
            <SportsSoccerIcon onClick={() => navigate('/staff')} sx={{ mr: 2, cursor: 'pointer' }} />
            <Typography
              variant="h6"
              noWrap
              component="a"
              ...
              <Tooltip title="Thông tin tài khoản">
                <Box sx={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }} onClick={handleOpenUserMenu}>
                  ...
                  <Typography sx={{ ml: 1, fontWeight: 600 }}>
                    {user ? `Nhân viên: ${user.lname} ${user.fname}` : 'Nhân viên'}
                  </Typography>
                </Box>
              </Tooltip>
